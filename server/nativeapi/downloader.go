...
func (api *Router) addDownloaderRoute(r chi.Router) {
	...
	r.Route("/dl", func(r chi.Router) {
		...
		r.Get("/album/{id}", dlAlbum(sc))
		r.Get("/artist/{id}", dlArtist(sc, lib))
		r.Post("/download", dlDownload(sc))
		r.Post("/import", dlImport(sc, lib, api.playlists))
		r.Post("/complete/preview", dlCompletePreview(sc))
		r.Post("/complete", dlComplete(sc))
		r.Get("/status", dlStatus(sc))
	})
}
...
func dlAlbum(sc *somedlClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, map[string]any{"id": id, "title": listing.Title, "tracks": normalizeTracks(listing.Tracks)})
	}
}
...
func dlDownload(sc *somedlClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req struct {
...
func dlComplete(sc *somedlClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req struct {
			AlbumID string `json:"albumId"`
			Present []int  `json:"present"`
...
func dlStatus(sc *somedlClient) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		writeJSON(w, normalizeStatus(resp))
	}
}
